...
const analyzeTicket = async (ticket: ITicket) => {
...
- Description: ${ticket.description}`);
  ...
  const raw = "content" in response.output[0] ? response.output[0].content : "";

  try {
    const match = String(raw).match(/```json\s*([\s\S]*?)\s*```/i);
    const jsonString = match ? match[1] : String(raw).trim();
